        ...
        if (type === 'deposit') {
            ...
            if (isUniSwapChecked) {
                if (uniShareBN.isZero()) {
                    setError(
                        `Please enter the amount of uniswap share to be deposited`
                    )
                    return
                }
                if (!uniShareBN.isZero() && uniShareBN.gt(uniShareBalanceBN)) {
                    setError(
                        `Deposited Uniswap share cannot exceed available amount.`
                    )
                    return
                }
            }
    ...
    return (
        <Body>
            ...
            {incentivesState.type === 'withdraw' ? (
                <SingleInput>
                    <DecimalInput
                        label={`UNI Pool Tokens (Avail ${formatNumber(
                            stakedBalance
                        )})`}
                        value={uniPool}
                        onChange={handleUniPoolChange}
                        handleMaxClick={() =>
                            handleUniPoolChange(stakedBalance)
                        }
                    />
                </SingleInput>
            ) : isChecked ? (
                ...
                <DoubleInput>
                    <DecimalInput
                        ...
                        } ETH (Avail ${formatNumber(ethBalance.toString())})`}
                        value={ethAmount}
                        onChange={handleChange}
                        handleMaxClick={() =>
                            handleChange(ethBalance.toString())
                        }
                    />
                    <DecimalInput
                        label={`${
            ...
            <UniSwapCheckContainer>
                ...
                <CheckBox
                    checked={isUniSwapChecked}
                    onChange={handleUniswapCheck}
                />
            </UniSwapCheckContainer>
            <Footer>
